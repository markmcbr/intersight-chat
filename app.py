...
# deploy via the DEFAULT_MODEL env var in docker-compose.yml or .env. Must
...
def handle_user_message(prompt: str) -> None:
    ...
    # Demo-prompt fast lane: a handful of high-value prompts (chassis
    # details, server details, alarm details) route through deterministic
    # Python gather + LLM-format-only pipelines, identical to the
    # Inventory Report preset. The model never sees Intersight tools on
    # these paths, so it can't mis-route, fabricate auth errors, hallucinate
    ...
    if demo_spec is not None:
        ...
        spec_for_user = dataclasses.replace(demo_spec, user_message=prompt)
        handle_preset_report(spec_for_user)
        return
